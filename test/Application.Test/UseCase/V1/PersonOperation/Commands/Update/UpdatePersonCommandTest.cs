...
using Moq;
using System.Net;
using onboardingback.Application.UseCase.V1.PersonOperation.Commands.Update;
...
namespace Application.Test.PersonOperation.Commands.Update
{
    public class UpdatePersonCommandTest
    {
        ...
        public async Task Handler_UpdatePerson_Success()
        {
            ...
            var person = new Fixture().Create<Pedido>();
            _query.Setup(_ => _.GetByIdAsync<Pedido>(It.IsAny<string>(), It.IsAny<string>())).ReturnsAsync(person);
        ...
        public async Task Handler_UpdatePerson_PersonNotExist()
        {
            // Arrange
            var request = new Fixture().Create<UpdatePersonCommand>();
            _query.Setup(_ => _.GetByIdAsync<Pedido>(It.IsAny<string>(), It.IsAny<string>()))
